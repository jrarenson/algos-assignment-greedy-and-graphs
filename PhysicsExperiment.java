...
public class PhysicsExperiment {
	...
	public int[][] scheduleExperiments(
		...
		int counter = 0, classmate = 0, max = 0;
		int step = 1;

		while (step <= numSteps) {
			for(int rows = 1; rows <= numStudents; rows++){
				for(int cols = step; cols <= numSteps; cols++){
					if(signUpTable[rows][cols] == 1) counter++; 
					else break;
				}
				if(counter > max){
					max = counter;
			...
			for(int x = step; x < step + max; x++){
				scheduleTable[classmate][x] = 1;
			}
